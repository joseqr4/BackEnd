...
EXEC sp_rename N'[Discounts].[discount_percentage]', N'discount_value', N'COLUMN';

GO

ALTER TABLE [Discounts] ADD [Article] decimal(2,1) NOT NULL DEFAULT 0.0;

GO

ALTER TABLE [Discounts] ADD [Discount_Type] nvarchar(max) NULL;

GO

ALTER TABLE [Discounts] ADD [Value] decimal(2,1) NOT NULL DEFAULT 0.0;

GO

CREATE TABLE [DiscountValued] (
    [IdDiscount] int NOT NULL IDENTITY,
    [DiscountsId] int NULL,
    [CountValued] int NOT NULL,
    CONSTRAINT [PK_DiscountValued] PRIMARY KEY ([IdDiscount]),
    CONSTRAINT [FK_DiscountValued_Discounts_DiscountsId] FOREIGN KEY ([DiscountsId]) REFERENCES [Discounts] ([Id]) ON DELETE NO ACTION
);
...
CREATE TABLE [QrCode] (
    ...
    [EmailUser] nvarchar(450) NOT NULL,
    [Id] int NOT NULL,
    [DiscountsId] int NULL,
    [EmailId] nvarchar(450) NULL,
    [Imagen] varbinary(max) NOT NULL,
    [TimeValidation] datetime2 NOT NULL,
    [Consumed] bit NOT NULL,
    [Valued] bit NOT NULL,
    CONSTRAINT [PK_QrCode] PRIMARY KEY ([EmailUser], [IdDiscount]),
    CONSTRAINT [AK_QrCode_Id] UNIQUE ([Id]),
    CONSTRAINT [FK_QrCode_Discounts_DiscountsId] FOREIGN KEY ([DiscountsId]) REFERENCES [Discounts] ([Id]) ON DELETE NO ACTION,
    CONSTRAINT [FK_QrCode_AspNetUsers_EmailId] FOREIGN KEY ([EmailId]) REFERENCES [AspNetUsers] ([Id]) ON DELETE NO ACTION
);

GO

CREATE INDEX [IX_DiscountValued_DiscountsId] ON [DiscountValued] ([DiscountsId]);

GO

CREATE INDEX [IX_QrCode_DiscountsId] ON [QrCode] ([DiscountsId]);

GO

CREATE INDEX [IX_QrCode_EmailId] ON [QrCode] ([EmailId]);

GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20200116234506_Disc', N'2.2.3-servicing-35854');
